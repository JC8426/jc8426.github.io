...
import {manualStep,automaticPose} from './flight-state.mjs';
...
function explore(on){
 ...
 renderer.domElement.style.touchAction=on?'none':'pan-y';controls.enabled=on&&state.mode!=='fpv';keys.clear();touch.clear();labels();if(on)host.focus({preventScroll:true});
}
function view(mode){
...
function syncTheme(){
 ...
 for(const u of units)u.root.position.y=Math.max(u.root.position.y,floor(u.root.position.x,u.root.position.z)+1.25);
 host.dataset.environment=state.dark?'moon':'desert';labels();render();
}
function cameraTargets(){
 const p=selected().root.position,yaw=selected().yaw;
 if(state.mode==='overview'){targetGoal.set(0,3,-4);cameraGoal.set(5,14,34);if(camera.aspect<.8)cameraGoal.multiplyScalar(1.35);}
...
function stepCamera(dt){
 ...
 else if(state.mode==='follow'){const delta=targetGoal.clone().sub(controls.target);camera.position.add(delta);controls.target.copy(targetGoal);}
 controls.update();
}
function updateHud(u,speed){
 ...
 $('[data-flight-state]').textContent=state.paused?tr('Paused','已暂停'):u.manual?tr('Manual control','手动操控'):tr('Formation flight','编队飞行');host.dataset.flight=state.paused?'paused':u.manual?'manual':'formation';
}
function animate(now){
 frame=requestAnimationFrame(animate);const dt=Math.min((now-(last||now))/1000,.05);last=now;
 if(!active||document.hidden||failed)return;if(!state.paused)clock+=dt*state.pace;let speed=0;
 for(const u of units){const before=u.root.position.clone();
  if(!state.paused){
   if(u.manual){const held=(k,c)=>keys.has(k)||touch.has(c);const input={forward:+held('w','forward')-held('s','back'),side:+held('d','right')-held('a','left'),up:+held('r','rise')-held('f','fall'),yaw:+held('q','yawLeft')-held('e','yawRight')};const v=manualStep(before,u.yaw,u===selected()&&state.exploring?input:{},dt,2*state.pace,floor,WORLD_LIMIT);u.root.position.set(v.x,v.y,v.z);u.yaw=v.yaw;}
...
function init(){
 ...
 keyLight=new T.DirectionalLight('#fff2dd',3);keyLight.position.set(-24,30,18);keyLight.castShadow=true;keyLight.shadow.mapSize.set(2048,2048);Object.assign(keyLight.shadow.camera,{left:-40,right:40,top:40,bottom:-40,near:.5,far:120});keyLight.shadow.bias=-.0002;keyLight.shadow.normalBias=.035;scene.add(keyLight);
...
$('[data-manual]').addEventListener('click',()=>{if(!state.ready)return;explore(true);selected().manual=!selected().manual;labels();updateHud(selected(),0);host.focus({preventScroll:true});});$('[data-rejoin]').addEventListener('click',()=>{if(!state.ready)return;selected().manual=false;keys.clear();touch.clear();labels();});
