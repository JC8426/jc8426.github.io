...
for(const root of document.querySelectorAll('[data-project-loop]')){
 ...
 prev.addEventListener('click',()=>step(-1));next.addEventListener('click',()=>step(1));play.addEventListener('click',()=>{playing=!playing;if(playing)step(1);updatePlay();});
 ...
 track.addEventListener('mouseenter',()=>{hovering=true;schedule();});track.addEventListener('mouseleave',()=>{hovering=false;schedule();});root.addEventListener('focusin',()=>{focused=document.activeElement!==play;schedule();});root.addEventListener('focusout',()=>queueMicrotask(()=>{focused=root.contains(document.activeElement)&&document.activeElement!==play;schedule();}));
